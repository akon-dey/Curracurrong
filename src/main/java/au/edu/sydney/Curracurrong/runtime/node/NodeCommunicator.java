...
public class NodeCommunicator extends Communicator {
     ...
     */
    private static NodeCommunicator instance=null;
     ...
     */
    public static NodeCommunicator getCommunicatorInstance() {
        if(instance == null) {
            ...
            if (sensorNodeMap == null ) {
                ...
                if (sensorNodeMap.get(src)!= null && sensorNodeMap.get(dest) != null) {
                    ...
                    if (sensorNodeMap.get(src).equals(sensorNodeMap.get(dest))) {
                        ...
                        System.out.println("About to execute fireStreamOperator()");
                        fireStreamOperator(so);
                        System.out.println("Done fireStreamOperator()");
                    } else {
                        System.out.println("About to execute propagateData(src.intValue(), dest.intValue(), data)");
                        propagateData(src.intValue(), dest.intValue(), data);
                        System.out.println("Done propagateData(src.intValue(), dest.intValue(), data)");
                    }
                } else {
    ...
    private void send(DRecord message) {
        try {
            ...
            if (sensorNodeMap.get(src) != null && sensorNodeMap.get(dest) != null) {
                ...
                System.out.println("dest [" + dest + "] operatorRefMap [" + operatorRefMap + "]");
...
//               conn = (RadiogramConnection) Connector.open("radiogram://broadcast:"+HOST_PORT);
               ...
               if(!dataBuff.isEmpty()) {
//            	   System.out.println("Data buff is not empty ...");
                   ...
                   while(keys.hasMoreElements()) {
                       ...
                       System.out.println("Data to be propagated from Node --> " + tmpData.tostring() + " No of Bytes -->" + tmpData.tostring().length());
                       try {
...
//                       DatagramPacket packet = new DatagramPacket(byteData, byteData.length, InetAddress.getByName("255.255.255.255"), HOST_PORT);
     ...
     */
    public void run() {
    	System.out.println("Communicator in Node Stared!!!");
        while (true) {
            try {
                if(!isConnectionClosed){
                    ...
                    System.out.println("receivedata element(1) = [" + receiveData.getElement(1).tostring() + "]");
                    System.out.println("sensor map = [" + sensorNodeMap + "]");
            		
                    try {
                        if(sensorNodeMap.containsKey(new Integer(receiveData.getElement(1).tostring()))) {
                            String receiverAddress = (sensorNodeMap.get(new Integer((receiveData.getElement(1)).tostring()))).toString();
                            System.out.println("Receiver =[" + receiverAddress + "] - myAddress = [" + myAddress + "]");
    ...
    void fireStreamOperator(StreamOperator so){
        synchronized(this){
            if(!Scheduler.getSchedulerInstance().isReady(so)){
                ...
                //notify();   //interferes communicator waiting thread
            }
        }
    }
   ...
   /**
    ...
    public synchronized void closeConnection() {
        try{
            ...
            System.out.println("radio connection is ON??" + !receiveSocket.isClosed());
            System.out.println("Communicator is going to wait at " + System.currentTimeMillis());
            wait();
            if(receiveSocket == null) {
                System.out.println("Communicator Notified but connection is closed");
                wait();
            }
        ...
        if (!isConnectionClosed && receiveSocket == null) {
            synchronized (this) {
                try {
                    ...
                    System.out.println("CASCADE: Error in opening the connection in Communicator" + e.getMessage());
                    e.printStackTrace();
                }
            }
